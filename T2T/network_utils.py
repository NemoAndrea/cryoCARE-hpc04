...
def predict_tomogram(model, tomogram, mean, std, tiles = (8,8)):
    ...
    print(model.config.axes[0:2])
    if (model.config.n_dim == 3):
         # axes for a slice network should be 'ZYX'
        tomo_denoised = denormalize(model.predict(tomogram, axes=model.config.axes, n_tiles=(8,8,8), normalizer=None), mean, std)
        ...
        for i in range(num_tomo_slices):
            ...
            # axes for a slice network should be 'XY'
